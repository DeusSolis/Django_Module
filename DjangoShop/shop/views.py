...
class PurchaseReturnCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        if purchase.time_of_buy + timedelta(seconds=60 * 3) < timezone.now():
            messages.error(self.request, 'Time to return 3 minutes has passed')
            return redirect('purchase')
        elif returns:
            messages.info(self.request, 'Return request accepted')
            return redirect('purchase')
        messages.info(self.request, 'Your request has been processed')
